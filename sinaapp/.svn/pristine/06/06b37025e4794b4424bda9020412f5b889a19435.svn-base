...
(function () {
	...
	CKEDITOR.event || (CKEDITOR.event = function () {}, CKEDITOR.event.implementOn = function (a) {
		...
		b.prototype = {
			getListenerIndex : function (b) {
				for (var a = 0, d = this.listeners; a < d.length; a++)
					if (d[a].fn == b)
						return a;
				return -1
			}
			...
			fire : function () {
				...
				return function (l, i, m) {
					...
					if (n) {
						var p = n.listeners;
						if (p.length)
							for (var p = p.slice(0), g, h = 0; h < p.length; h++) {
								if (n.errorProof)
									try {
										g = p[h].call(this,
												m, i, a, j)
	...
	(function () {
		...
		CKEDITOR.tools = {
			...
			extend : function (b) {
				...
				e,
				d;
				if (typeof(e = arguments[a - 1]) == "boolean")
					a--;
			...
			cssStyleToDomStyle : function () {
				...
				a = typeof b.cssFloat != "undefined" ? "cssFloat" :
			...
			buildStyleHtml : function (b) {
				for (var b = [].concat(b), a, e = [], d = 0; d < b.length; d++)
					if (a = b[d])
						 /  @ import | [{}
			...
			normalizeCssText : function (b,
				...
				var e = [],
				d,
				c = CKEDITOR.tools.parseCssText(b, true, a);
				for (d in c)
					e.push(d + ":" + c[d]);
				e.sort();
			...
			convertRgbToHex : function (b) {
				return b.replace(/(?:rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\))/gi, function (b, a, d, c) {
					b = [a, d, c];
					for (a = 0; a < 3; a++)
						b[a] = ("0" + parseInt(b[a], 10).toString(16)).slice(-2);
					return "#" + b.join("")
				})
			},
	...
	CKEDITOR.tools.extend(CKEDITOR.dom.node.prototype, {
		...
		clone : function (a, c) {
			...
			f = function (b) {
				...
				if (b.nodeType == CKEDITOR.NODE_ELEMENT) {
					...
					if (a)
						for (var b = b.childNodes, d = 0; d < b.length; d++)
							f(b[d])
				}
			};
			f(b);
		...
		getIndex : function (a) {
			...
			if (!this.$.parentNode)
				return b;
			do
				if (!a || !(c != this.$ && c.nodeType == CKEDITOR.NODE_TEXT && (f || !c.nodeValue))) {
					b++;
		...
		getPrevious : function (a) {
			...
			b;
			do
				b = (c = c.previousSibling) && c.nodeType != 10 && new CKEDITOR.dom.node(c);
			while (b && a && !a(b));
			return b
		},
		getNext : function (a) {
			...
			b;
			do
				b = (c = c.nextSibling) && new CKEDITOR.dom.node(c);
			while (b && a && !a(b));
			return b
		},
		...
		getPosition : function (a) {
			...
			if (c.compareDocumentPosition)
				return c.compareDocumentPosition(b);
			if (c == b)
			...
			if (this.type == CKEDITOR.NODE_ELEMENT && a.type == CKEDITOR.NODE_ELEMENT) {
				...
				if ("sourceIndex" in c)
					return c.sourceIndex < 0 || b.sourceIndex < 0 ? CKEDITOR.POSITION_DISCONNECTED : c.sourceIndex < b.sourceIndex ? CKEDITOR.POSITION_PRECEDING : CKEDITOR.POSITION_FOLLOWING
			}
			for (var c = this.getAddress(), a = a.getAddress(), b = Math.min(c.length, a.length), f = 0; f <= b - 1; f++)
				if (c[f] != a[f]) {
					if (f < b)
						return c[f] < a[f] ? CKEDITOR.POSITION_PRECEDING : CKEDITOR.POSITION_FOLLOWING;
					break
				}
		...
		getAscendant : function (a, c) {
			...
			if (!c)
				b = b.parentNode;
			for (; b; ) {
				if (b.nodeName && (f = b.nodeName.toLowerCase(), typeof a == "string" ? f == a : f in a))
					return new CKEDITOR.dom.node(b);
				try {
					b = b.parentNode
				} catch (e) {
		...
		remove : function (a) {
			...
			if (b) {
				if (a)
					for (; a = c.firstChild; )
						b.insertBefore(c.removeChild(a), c);
				b.removeChild(c)
			}
			return this
		},
		...
		ltrim : function () {
			for (var a; this.getFirst && (a = this.getFirst()); ) {
				if (a.type == CKEDITOR.NODE_TEXT) {
					var c = CKEDITOR.tools.ltrim(a.getText()),
					b = a.getLength();
					if (c) {
						if (c.length < b) {
							a.split(b - c.length);
		...
		rtrim : function () {
			for (var a; this.getLast && (a =
						this.getLast()); ) {
				if (a.type == CKEDITOR.NODE_TEXT) {
					var c = CKEDITOR.tools.rtrim(a.getText()),
					b = a.getLength();
					if (c) {
						if (c.length < b) {
							a.split(c.length);
	...
	CKEDITOR.tools.extend(CKEDITOR.dom.document.prototype, {
		...
		getByAddress : function (a, c) {
			for (var b = this.$.documentElement, f =
				...
				var e = a[f];
				if (c)
					for (var d = -1, j = 0; j < b.childNodes.length; j++) {
	...
	CKEDITOR.dom.element.clearAllMarkers = function (a) {
		for (var c in a)
			CKEDITOR.dom.element.clearMarkers(a, a[c], 1)
	};
	...
	(function () {
		function a(b) {
			for (var a = 0, e = 0, d = c[b].length; e < d; e++)
				a = a + (parseInt(this.getComputedStyle(c[b][e]) || 0, 10) || 0);
			return a
		}
		CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
			...
			removeClass : function (b) {
				...
				if (a) {
					b = RegExp("(?:^|\\s+)" + b + "(?=\\s|$)", "i");
					if (b.test(a))
						(a = a.replace(b, "").replace(/^\s+/, "")) ? this.setAttribute("class",
			...
			appendBogus : function () {
				for (var b = this.getLast(); b && b.type == CKEDITOR.NODE_TEXT && !CKEDITOR.tools.rtrim(b.getText()); )
					b = b.getPrevious();
				if (!b || !b.is || !b.is("br")) {
			...
			mergeSiblings : function () {
				function b(b, a, c) {
					if (a && a.type == CKEDITOR.NODE_ELEMENT) {
						...
						if (b.isIdentical(a)) {
							for (var l = c ? b.getLast() : b.getFirst(); j.length; )
								j.shift().move(b, !c);
							a.moveChildren(b, !c);
							a.remove();
			...
			removeStyle : function (b) {
				...
				if (!a.removeProperty && (b == "border" || b == "margin" || b == "padding")) {
					...
					for (var a = [], j = 0; j < c.length; j++)
						if (d)
							for (var l = 0; l < d.length; l++)
								a.push([b, c[j], d[l]].join("-"));
						else
							a.push([b, c[j]].join("-"));
			...
			getDocumentPosition : function (b) {
				...
				if (document.documentElement.getBoundingClientRect) {
					...
					m = d.$.documentElement,
					n = m.clientTop || j.$.clientTop || 0,
					r = m.clientLeft || j.$.clientLeft || 0,
					p = true;
					if (CKEDITOR.env.ie) {
			...
			scrollIntoParent : function (b, a, c) {
				...
				i;
				function m(a, c) {
					if (/body|html/.test(b.getName()))
						b.getWindow().$.scrollBy(a, c);
					else {
				...
				if (i < 0 || j > 0)
					m(0, a === true ? i : a === false ? j : i < 0 ? i : j);
				if (c && (l < 0 || d > 0))
					m(l < 0 ? l : d, 0)
			},
			...
			copyAttributes : function (b, a) {
				for (var c = this.$.attributes, a = a || {}, d = 0; d < c.length; d++) {
					var j = c[d],
					l = j.nodeName.toLowerCase(),
					i;
					if (!(l in a))
						if (l == "checked" && (i = this.getAttribute(l)))
							b.setAttribute(l, i);
						else if (j.specified || CKEDITOR.env.ie && j.nodeValue && l == "value") {
				...
				if (this.$.style.cssText !== "")
					b.$.style.cssText = this.$.style.cssText
			},
	...
	(function () {
		function a(a, b) {
			...
			g = h ? function (a, b) {
			 ...
			 : B;
			if (this.current)
				d = this.current[f](false, u, g);
			else {
				if (a)
					d.type ==
					CKEDITOR.NODE_ELEMENT && (d = p > 0 ? d.getChild(p - 1) : g(d, true) === false ? null : d.getPreviousSourceNode(true, u, g));
				else {
					d = n;
					if (d.type == CKEDITOR.NODE_ELEMENT && !(d = d.getChild(r)))
						d = g(n, true) === false ? null : n.getNextSourceNode(true, u, g)
				}
				d && g(d) === false && (d = null)
			}
			for (; d && !this._.end; ) {
	...
	(function () {
		...
		e = function (a, b, c, g) {
			...
			if (h.type == CKEDITOR.NODE_TEXT) {
				...
				if (f >= h.getChildCount()) {
					h = h.append(a.document.createText(""));
					e = true
				} else
					h = h.getChild(f).getPrevious();
			else {
			...
			for (var m = c, s, A, v, o = l; o < f.length; o++) {
				...
				for (s = s.getNext(); s; ) {
					if (s.equals(k[o]) || s.equals(d))
						break;
					v = s.getNext();
					if (b == 2)
						m.append(s.clone(true));
					else {
						s.remove();
						b == 1 && m.append(s)
					}
			...
			for (c = l; c < k.length; c++) {
				...
				if (!f[c] || s.$.parentNode != f[c].$.parentNode)
					for (s = s.getPrevious(); s; ) {
						if (s.equals(f[c]) || s.equals(h))
							break;
						v = s.getPrevious();
						if (b == 2)
							m.$.insertBefore(s.$.cloneNode(true), m.$.firstChild);
						else {
							s.remove();
		...
		CKEDITOR.dom.range.prototype = {
			...
			moveToBookmark : function (a) {
				if (a.is2) {
					...
					c = a.startOffset,
					g = a.end && this.document.getByAddress(a.end, a.normalized),
					a = a.endOffset;
					this.setStart(b, c);
			...
			getBoundaryNodes : function () {
				...
				if (b.type == CKEDITOR.NODE_ELEMENT) {
					h = b.getChildCount();
					if (h > g)
						b = b.getChild(g).getPreviousSourceNode(true);
					else if (h < 1)
			...
			enlarge : function (a, b) {
				...
				case CKEDITOR.ENLARGE_ELEMENT:
					...
					for (; k || e; ) {
						if (k && !e) {
							!j && k.equals(g) && (j = true);
							if (c ? k.isBlockBoundary() : !h.contains(k))
								break;
						...
						for (; e; ) {
							i = false;
							if (e.type == CKEDITOR.NODE_COMMENT)
								e = e.getPrevious();
							else {
								if (e.type == CKEDITOR.NODE_TEXT) {
									...
									if (l && CKEDITOR.dtd.$removeEmpty[e.getName()]) {
										q =
											e.getText();
										if (/[^\s\ufeff]/.test(q))
											e = null;
										else
											for (var m = e.$.getElementsByTagName("*"), s = 0, A; A = m[s++]; )
			...
			shrink : function (a, b, c) {
				if (!this.collapsed) {
					...
					g.guard = function (b, g) {
						if (l(b))
							return true;
						if (a == CKEDITOR.SHRINK_ELEMENT && b.type == CKEDITOR.NODE_TEXT || g && b.equals(m) || c === false && b.type == CKEDITOR.NODE_ELEMENT && b.isBlockBoundary())
							return false;
						!g && b.type == CKEDITOR.NODE_ELEMENT && (m = b);
						return true
					};
					if (j)
						(h = g[a == CKEDITOR.SHRINK_ELEMENT ? "lastForward" :
			...
			fixBlock : function (a, b) {
				...
				g = this.document.createElement(b);
				this.collapse(a);
				...
				for (var d = 0; a; ) {
					...
					if (a.type == CKEDITOR.NODE_ELEMENT)
						if (a.isEditable()) {
							...
							c && (a.is("br") && this.checkEndOfBlock()) && this.moveToPosition(a, CKEDITOR.POSITION_BEFORE_START);
					var g = a,
					h = d,
					e = void 0;
	...
	(function () {
		...
		a.prototype = {
			getNextParagraph : function (a) {
				a = a || "p";
				if (!CKEDITOR.dtd[this.range.root.getName()][a])
					return null;
				var e,
				i,
				m,
				...
				for (this._.nextNode = null; g; ) {
					var h = 0,
					u = g.hasAscendant("pre"),
					w = g.type != CKEDITOR.NODE_ELEMENT,
					k = 0;
					if (w)
						g.type == CKEDITOR.NODE_TEXT && b.test(g.getText()) && (w = 0);
					else {
						var t = g.getName();
						if (g.isBlockBoundary(this.forceBrBreak && !u && {
								br : 1
							})) {
							if (t == "br")
								w = 1;
							else if (!i && !g.getChildCount() && t != "hr") {
								e = g;
								m = g.equals(n);
								break
							}
				...
				if (!e) {
					...
					e = e.block;
					if (!e && g && !this.enforceRealBlocks && h[g.getName()] && i.checkStartOfBlock() && i.checkEndOfBlock() && !g.equals(i.root))
						e = g;
						...
						this._.nextNode = e.equals(n) ? null : c(i.getBoundaryNodes().endNode, 1, n)
				}
				if (r)
					(i = e.getPrevious()) && i.type == CKEDITOR.NODE_ELEMENT && (i.getName() == "br" ? i.remove() : i.getLast() && i.getLast().$.nodeName.toLowerCase() == "br" && i.getLast().remove());
				if (p)
					(i = e.getLast()) && i.type == CKEDITOR.NODE_ELEMENT && i.getName() == "br" && (CKEDITOR.env.ie || i.getPrevious(f) || i.getNext(f)) && i.remove();
				if (!this._.nextNode)
					this._.nextNode = m ||
	...
	CKEDITOR.command.prototype = {
		...
		setState : function (a) {
			if (this.state == a || !this.checkAllowed())
				return false;
	...
	(function () {
					...
					a : if (a.nothingRequired)
						...
						if (h = a.requiredClasses) {
							...
							for (g = 0; g < h.length; ++g)
								if (CKEDITOR.tools.indexOf(f, h[g]) == -1) {
									g = false;
									break a
								}
						}
						...
						for (var e = h.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || h.name == "body", j, f = d.length; f > 0; ) {
							...
							if (e && (k.type == CKEDITOR.NODE_TEXT || k.type == CKEDITOR.NODE_ELEMENT && D.$inline[k.name])) {
								...
								h.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT && (k.type ==
									CKEDITOR.NODE_ELEMENT && !D[h.name][k.name]) && c.push({
									check : "el-up",
									el : k
								})
							}
		...
		CKEDITOR.filter.prototype = {
			...
			applyTo : function (a, b, c) {
				var d = [],
				g = !c && this._.rules,
				e = this._.transformations,
				k = f(this),
				j = this.editor && this.editor.config.protectedSource,
				u = false;
						...
						a : {
							...
							if (j)
								for (o = 0; o < j.length; ++o)
									if ((p = h.match(j[o])) && p[0].length == h.length) {
										c = true;
										break a
									}
				...
				for (var o, p, a = [], c = ["p", "br", "div"][this.enterMode - 1]; o = d.pop(); )
					o.type == CKEDITOR.NODE_ELEMENT ? h(o, c, a) : o.remove();
				for (; p = a.pop(); ) {
					o = p.el;
					if (o.parent)
						switch (p.check) {
	...
	(function () {
		CKEDITOR.lang = {
			...
			detect : function (a, c) {
				...
				c = c || navigator.userLanguage || navigator.language || a,
	...
	CKEDITOR.resourceManager.prototype = {
		...
		load : function (a, c, b) {
			...
			CKEDITOR.scriptLoader.load(d, function (a,
					d) {
				if (d.length)
					throw '[CKEDITOR.resourceManager.load] Resource name "' + j[d[0]].join(",") + '" was not found at "' + d[0] + '".';
				for (var g = 0; g < a.length; g++)
					for (var h = j[a[g]], e = 0; e < h.length; e++) {
						var i = h[e];
						l[i] = this.get(i);
						f[i] = 1
					}
				...
				j = function (b) {
					a.call(this, b, function (a) {
						...
						for (l in a) {
							...
							if (!c[l]) {
								if (r.icons)
									for (var g = r.icons.split(","), h = 0; h < g.length; h++)
										CKEDITOR.skin.addIcon(g[h], r.path + "icons/" + g[h] + ".png");
								c[l] = 1
							}
							if (p) {
								p.split && (p = p.split(","));
	...
	CKEDITOR.ui.prototype = {
		...
		create : function (a) {
			var c = this.items[a],
			b = c && this._.handlers[c.type],
			f = c && c.command && this.editor.getCommand(c.command),
			b = b && b.create.apply(this, c.args);
			this.instances[a] = b;
			f && f.uiItems.push(b);
			if (b && !b.type)
				b.type = c.type;
	...
	(function () {
		function a(a, d, g) {
			...
			if (d !== void 0) {
				...
				if (CKEDITOR.env.ie && CKEDITOR.env.quirks && g == CKEDITOR.ELEMENT_MODE_INLINE)
		...
		function n(a) {
			...
			if (g)
				var f = RegExp("(?:^|,)(?:" + g.replace(/\s*,\s*/g, "|") + ")(?=,|$)", "g"), c = c.replace(f, "");
			...
			CKEDITOR.plugins.load(c.split(","), function (c) {
				...
				for (var k in c) {
					...
					if (o && !a.lang[k]) {
						...
						if (!j.langEntries || !j.langEntries[p])
							e.push(CKEDITOR.getUrl(j.path + "lang/" + p + ".js"));
						else {
				...
				CKEDITOR.scriptLoader.load(e,
					...
					for (k = 0; k < a.config.blockedKeystrokes.length; k++)
						a.keystrokeHandler.blockedKeystrokes[a.config.blockedKeystrokes[k]] = 1;
					a.status = "loaded";
					a.fireOnce("loaded");
	...
	(function () {
		...
		CKEDITOR.htmlParser.node.prototype = {
			remove : function () {
				...
				c = CKEDITOR.tools.indexOf(a, this),
				b = this.previous,
				f = this.next;
				b && (b.next = f);
				f && (f.previous = b);
			...
			replaceWith : function (a) {
				...
				b = CKEDITOR.tools.indexOf(c, this),
				f = a.previous = this.previous,
				e = a.next = this.next;
				f && (f.next = a);
				e && (e.previous = a);
				c[b] = a;
	...
	(function () {
		...
		CKEDITOR.htmlParser.fragment.fromHtml = function (e, d, j) {
				...
				if (a(e))
					...
					if (d == "br")
						...
						for (; ; ) {
							...
							if (!e.isUnknown && !k.isUnknown && !j[d])
								if (k.isOptionalClose)
									...
									k = h
								} else if (d in CKEDITOR.dtd.$listItem && !p(d, h))
									g.onTagOpen(d == "li" ? "ul" : "dl", {}, 0, 1);
								else if (h in c && !p(d, h)) {
									!e.returnPoint &&
									(e.returnPoint = k);
									k = k.parent
								} else {
			...
			g.onText = function (a) {
				...
				var d = k.name,
				e = d ? CKEDITOR.dtd[d] || (k._.isBlockLike ? CKEDITOR.dtd.div : CKEDITOR.dtd.span) : f;
				if (!t && !e["#"] && d in c) {
	...
	(function () {
		function a(a, c) {
				...
				return function (e) {
					if (e.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
						...
						for (o =
								0; o < h.length; o++)
							i(h[o]);
						if (h = CKEDITOR.env.opera && !a || (typeof c == "function" ? c(e) !== false : c))
							if (!p && CKEDITOR.env.ie && e.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT)
								h = false;
							else if (!p && CKEDITOR.env.ie && (document.documentMode > 7 || e.name in CKEDITOR.dtd.tr || e.name in CKEDITOR.dtd.$listItem))
								h = false;
							else {
								h = b(e);
			...
			function k(a, b) {
				...
				if (a.type ==
					...
					if (CKEDITOR.env.ie && p && (!b || a.parent.name in r))
						return true;
					if (!p)
			...
			p = c == "html",
			r = CKEDITOR.tools.extend({}, z),
			u;
			for (u in r)
				"#" in B[u] || delete r[u];
			for (u in r)
				o.elements[u] = h(p, a.config.fillEmptyBlocks !== false);
			o.root = h(p);
		...
		function t(a, b) {
			for (var c = [], g = b.config.protectedSource, d = b._.dataStore || (b._.dataStore = {
							id : 1
						}), e = /<\!--\{cke_temp(comment)?\}(\d*?)--\>/g, g = [/<script[\s\S]*?<\/script>/gi, /<noscript[\s\S]*?<\/noscript>/gi].concat(g), a = a.replace(/<\!--[\s\S]*?--\>/g, function (a) {
						return "<\!--{cke_tempcomment}" + (c.push(a) - 1) + "--\>"
					}), h = 0; h < g.length; h++)
				a = a.replace(g[h], function (a) {
	...
	(function () {
		...
		CKEDITOR.htmlParser.element.prototype = CKEDITOR.tools.extend(new CKEDITOR.htmlParser.node, {
				...
				replaceWithChildren : function () {
					for (var a = this.children, c = a.length; c; )
						a[--c].insertAfter(this);
					this.remove()
				},
				forEach : c.forEach
			})
	...
	(function () {
		...
		CKEDITOR.inlineAll = function () {
			...
			for (b in CKEDITOR.dtd.$editable)
				for (var f = CKEDITOR.document.getElementsByTag(b), e = 0, d = f.count(); e < d; e++) {
					a = f.getItem(e);
					if (a.getAttribute("contenteditable") == "true") {
						c = {
							element : a,
	...
	(function () {
		function a(a) {
			...
			d = a.data.path,
			e = d.blockLimit,
			f = a.data.selection.getRanges()[0],
			k = c.config.enterMode;
			if (CKEDITOR.env.gecko) {
				var i = d.block || d.blockLimit || d.root,
				j = i && i.getLast(b);
			...
			if (c.config.autoParagraph !== false && k != CKEDITOR.ENTER_BR && f.collapsed && g.equals(e) && !d.block) {
				...
				if (!d.checkForward() || g.checkStartOfBlock() && g.checkEndOfBlock()) {
					...
					if (CKEDITOR.env.ie)
						(c = c.getFirst(b)) && (c.type == CKEDITOR.NODE_TEXT && CKEDITOR.tools.trim(c.getText()).match(/^(?:&nbsp;|\xa0)$/)) && c.remove();
					f.select();
					a.cancel()
				}
			}
				...
				proto : {
					...
					insertText : function (a) {
						...
						if (!b) {
							var f = /\n{2}/g;
							if (f.test(a))
								var i = "<" + c + ">", k = "</" + c + ">", a = i + a.replace(f, function () {
										return k + i
									}) + k
						}
						a = a.replace(/\n/g, "<br>");
					...
					insertElement : function (a) {
						...
						for (var c = this.editor, g = c.config.enterMode,
							...
							if (!m.checkReadOnly()) {
								...
								z;
								if (k)
									for (; (q = m.getCommonAncestor(0, 1)) && (z = CKEDITOR.dtd[q.getName()]) && (!z || !z[i]); )
										if (q.getName()in CKEDITOR.dtd.span)
											m.splitElement(q);
											...
											m.splitBlock(g == CKEDITOR.ENTER_DIV ? "div" : "p", c.editable());
								m.insertNode(n);
								l ||
								(l = n)
							}
						...
						if (l) {
							...
							if (k)
								if ((a = l.getNext(b)) && a.type == CKEDITOR.NODE_ELEMENT && a.is(CKEDITOR.dtd.$block))
									a.getDtd()["#"] ? m.moveToElementEditStart(a) : m.moveToElementEditEnd(l);
								else if (!a && g != CKEDITOR.ENTER_BR) {
							...
							this.attachListener(a, "key", function (b) {
								...
								if (c in {
									...
									if (d = e(d)) {
										...
										if ((f = h.block) && b[c ? "checkStartOfBlock" : "checkEndOfBlock"]() && (j = f[c ? "getPrevious" : "getNext"](i)) && j.is("table")) {
											a.fire("saveSnapshot");
											...
											a.fire("saveSnapshot");
											g = 1
										} else if ((p = h.contains(["td", "th", "caption"])) && b.checkBoundaryOfElement(p, c ? CKEDITOR.START : CKEDITOR.END))
											g = 1
								}
								return !g
							});
		...
		var n = function () {
			...
			return function (m, q, t) {
				var s = m.editor;
				m.getDocument();
				var A = s.getSelection().getRanges()[0],
				v = false;
				...
				if (!A.checkReadOnly()) {
					...
					for (; (C = a(s.startContainer) && s.startContainer.getChild(s.startOffset - 1)) && a(C) && C.isBlockBoundary() && I.contains(C); )
						s.moveToPosition(C, CKEDITOR.POSITION_BEFORE_END);
					...
					for (o = x.startNode; (o = o.getParent()) && e(o); )
						v.push(o);
					s.moveToBookmark(x);
					if (t) {
						C = t;
						...
						if ((L = t.startPath().block) && !(L.getChildCount() == 1 && L.getBogus()))
							a : {
								var E;
					...
					for (P = J.createBookmark(); H = q.zombies.pop(); )
						if (H.getParent()) {
							N = J.clone();
	...
	(function () {
		...
		var l,
		i,
		m = CKEDITOR.dom.walker.invisible(1);
		CKEDITOR.on("instanceCreated", function (b) {
			...
			e.on("contentDom", function () {
				...
				if (p ? CKEDITOR.env.webkit || CKEDITOR.env.gecko : CKEDITOR.env.opera) {
					...
					f.attachListener(CKEDITOR.env.ie ? f : b.getDocumentElement(), "mouseup", c, e);
			...
			getType : n ? function () {
			 ...
			 : function () {
				...
				if (!c || !c.rangeCount)
					...
					d = c.startContainer;
					if (d == c.endContainer && d.nodeType == 1 && c.endOffset - c.startOffset == 1 && r[d.childNodes[c.startOffset].nodeName.toLowerCase()])
						b =
			...
			getSelectedElement : function () {
				...
				c = CKEDITOR.tools.tryThese(function () {
						...
						for (var a = b.getRanges()[0], c, d, e = 2; e && (!(c = a.getEnclosedNode()) || !(c.type ==
									CKEDITOR.NODE_ELEMENT && r[c.getName()] && (d = c))); e--)
				...
				if (a.length)
					if (this.isLocked) {
						...
						if (n) {
							...
							m,
							l = b.getEnclosedNode();
							if (l && l.type == CKEDITOR.NODE_ELEMENT && l.getName()in r && (!l.is("a") || !l.getText()))
								try {
	...
	CKEDITOR.editor.prototype.attachStyleStateChange = function (a, c) {
		...
		if (!b) {
			...
			this.on("selectionChange", function (a) {
				for (var c = 0; c < b.length; c++) {
					var d = b[c],
					j = d.style.checkActive(a.data.path) ? CKEDITOR.TRISTATE_ON : CKEDITOR.TRISTATE_OFF;
					d.fn.call(this, j)
				}
			})
		}
	...
	(function () {
			...
			if (b.collapsed) {
				...
				for (l.getPosition(m) == CKEDITOR.POSITION_FOLLOWING && (l = 0); l; ) {
					...
					if (l.equals(m)) {
						...
						var q = l.type,
						s = q == CKEDITOR.NODE_ELEMENT ? l.getName() : null,
						...
						for (var g = u(this, c), r = g.hasAttributes(), t = n.getCommonAncestor(), q = {}, s = {}, v = {}, D = {}, B, z, F; g && t; ) {
							if (t.getName() == d) {
								for (B in e.attributes)
									if (!D[B] && (F = t.getAttribute(z)))
										g.getAttribute(B) == F ? s[B] = 1 : D[B] = 1;
								for (z in e.styles)
									if (!v[z] && (F = t.getStyle(z)))
										g.getStyle(z) == F ? q[z] = 1 : v[z] = 1
							}
							t = t.getParent()
						}
						for (B in s)
		...
		function e(a) {
			...
			if (a = (new CKEDITOR.dom.elementPath(b, a.root)).contains(this.element, 1)) {
				...
				if (c)
					for (var e in c)
						a.removeAttribute(e, c[e]);
				if (b.styles)
					for (var d in b.styles)
						b.styles.hasOwnProperty(d) && a.removeStyle(d)
			}
		}
		function d(a) {
			...
			for (var e, d = a.document; e = c.getNextParagraph(); )
				if (!e.isReadOnly()) {
					var f = u(this, d, e);
					l(e, f)
				}
		...
		function j(a) {
			...
			for (var e; e = c.getNextParagraph(); )
				if (this.checkElementRemovable(e))
					if (e.is("pre")) {
		...
		function l(a, b) {
			...
			if (e && !d) {
				...
				f ? b = n(c ? [a.getHtml()] : i(a), b) : a.moveChildren(b);
			b.replace(a);
			if (e) {
				var c = b,
				j;
		...
		function r(a) {
			...
			for (var i in b)
				if (!(this._.definition.fullMatch && a.getStyle(i) != F(i, b[i], true))) {
					e = e || !!a.getStyle(i);
					a.removeStyle(i)
				}
			g(a, d, B[a.getName()]);
		...
		function k(a, b) {
			for (var c in a)
				a[c] = a[c].replace(s, function (a, c) {
						return b[c]
					})
		}
		function t(a) {
	...
	CKEDITOR.editor.prototype.getStylesSet = function (a) {
		if (this._.stylesDefinitions)
			...
			var c = this,
			b = c.config.stylesCombo_stylesSet || c.config.stylesSet;
			if (b === false)
				a(null);
	...
	(function () {
		...
		for (c in CKEDITOR.dtd.$blockLimit)
			c in CKEDITOR.dtd.$list || (a[c] = 1);
		var b = {};
		for (c in CKEDITOR.dtd.$block)
			c in CKEDITOR.dtd.$blockLimit || c in CKEDITOR.dtd.$empty || (b[c] = 1);
	...
	(function () {
		function a(a, c, e) {
			...
			j = c[e ? "endContainer" : "startContainer"],
					...
					if (!(3 > arguments.length)) {
						...
						CKEDITOR.ui.dialog.labeledElement.call(this, b, a, d, function () {
							...
							for (var k = 0, j; k < a.items.length && (j = a.items[k]); k++)
								i.push('<option value="', CKEDITOR.tools.htmlEncode(j[1] !== void 0 ? j[1] : j[0]).replace(/"/g, "&quot;"), '" /> ', CKEDITOR.tools.htmlEncode(j[0]));
							if (typeof c.inputStyle != "undefined")
								c.style = c.inputStyle;
	...
	(function () {
		...
		function V(a) {
			...
			function e(d) {
				...
				if ((e ? x : y.x) + E > h.width)
					E = h.width - (e ? x : y.x);
				if (f == CKEDITOR.DIALOG_RESIZE_WIDTH ||
					f == CKEDITOR.DIALOG_RESIZE_BOTH)
					i = Math.max(g.minWidth || 0, E - m);
			...
			function d(b) {
				if (l ==
					...
					if (9 == c || c == CKEDITOR.SHIFT + 9)
						c = c == CKEDITOR.SHIFT + 9, l._.tabBarMode ? (c = c ? p.call(l) : u.call(l), l.selectPage(c), l._.tabs[c][0].focus()) : e(c ? -1 : 1), o = 1;
						...
						c = c == (d ? 39 : 37) ? p.call(l) : u.call(l), l.selectPage(c), l._.tabs[c][0].focus(), o = 1;
		...
		M = function (a) {
			...
			if ((b = t[(b ? "CTRL+" : "") + (c ? "ALT+" : "") + (e ? "SHIFT+" : "") + d]) && b.length)
	...
	(function () {
		function w(a) {
			...
			function h(b) {
				...
				for (e = 0; e < b.length; ++e)
					d = b[e], h = {},
				"/" == d ? a.push(d) : CKEDITOR.tools.isArray(d) ? (c(h, CKEDITOR.tools.clone(d)), a.push(h)) : d.items && (c(h, CKEDITOR.tools.clone(d.items)),
			...
			r = CKEDITOR.tools.addFunction(function (b, e) {
					...
					case f ? 37:
						...
						return (f = CKEDITOR.document.getById(c + (b - 1))) || (f = CKEDITOR.document.getById(c +
									(a._.elementsPath.list.length - 1))),
						f.focus(),
						!1;
					case 27:
						return a.focus(),
	...
	(function () {
		function n(a) {
			...
			if (c.config.disableObjectResizing)
				try {
		...
		function q() {
			...
			if (8 <= CKEDITOR.document.$.documentMode) {
				...
				for (d in CKEDITOR.dtd.$removeEmpty)
					c.push("html.CSS1Compat " + d + "[contenteditable=false]");
	...
	CKEDITOR.removeAnchorCommand.prototype = {
		exec : function (b) {
			...
			if (a && (c = a.getSelectedElement()) && (CKEDITOR.plugins.link.fakeAnchor && !c.getChildCount() ? CKEDITOR.plugins.link.tryRestoreFakeAnchor(b, c) : c.is("a")))
				...
				c.hasAttribute("href") ? (c.removeAttributes({
						name : 1,

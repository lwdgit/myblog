...
$db->query($sql);
$sql="delete from photos where ablumid='$_GET[ablum_id]'";
$db->query($sql);
...
function edit_article(id,ablumid)
{
$.get('photo_edit.php',{articleid:id,ablum_id:ablumid},function (resultData){
$('#main').html(resultData);
},'text');
}
function add_article()
{
$.get('photo_edit.php','',function (resultData){
$('#main').html(resultData);
},'text');
}
function del_article(id,ablumid)
{
if(confirm("确实要删除吗？")==1)
$.get('photo_m.php',{del_article_id:id,ablum_id:ablumid},function (resultData){
$('#main').html(resultData);
},'text');
}
function getList(pageid)
{
$.get('photo_m.php',{page:pageid},function (resultData){
$('#main').html(resultData);
},'text');
}
...
<h3 class="hndle">
                        ...
                        <span id="edit-playlist-actions">
                            <input id="add-new-art" name="add-new-art" type="button"  title="新建相册" value="新建"  onclick="add_article();">
                        </span>
</h3>
        ...
        <div id="article-list" class="list-group">
            ...
            ?>
              ...
              <a href="javascript:;" onclick="edit_article('<? echo $row['id'].'\',\''.$row['ablumid'];?>');">编辑</a>
              <a href="javascript:;" onclick="del_article('<? echo $row['id'].'\',\''.$row['ablumid'];?>');">删除</a>
              </span>
              </li>               
            <? }?>          
          </ul>

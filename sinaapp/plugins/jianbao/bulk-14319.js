...
/* ======================= */
	...
	$('#bodyContent').html(''
		...
		+				($R.embedded && $R.embeddedOptions['logo'] > '' ?
						...
						'<a href="#">'+$R.win.location.href+'</a>')
		+			'<form name="__art_form" id="__art_form" method="post" action="">'
		+			'<input id="art_src" name ="src" type="text" value='+$R.win.location.href+'>'
		+			'<input id="__art_title" name ="tl" type="text" value="title">'
		+			'<input id="__art_tag" name ="nb" type="text" value="cnet|">'
        +			'<button id="__art_submit" name="_submit">submit</button>'
        +			'<button id="__art_download" name="_download">download</button>'
		+			'<textarea id="__art_content" name="bs" class="xheditor-mini" rows="12" cols="280" style="width: 100%;height:700px"></textarea>'
		+			'</form>'
		+			'</div>'
		...
		+	'<a id="fitts" href="#" title="Click to close"></a>'
		+	'<div id="floating">'
...
/* ======== */
	...
	function preSubmit(arg)
	{
		$R.article_content[0].innerHTML =$R.editor.getSource();
		document.__art_form.action=$R.linksPath+'admin/postdata3.php'+arg;
		document.__art_form.submit();
	}
    ...
    $('#__art_submit').click(function(){preSubmit('');return false; });        
    $('#__art_download').click(function(){preSubmit('?word=1');return false; }); 
	...
	$R.setOptions = function ()
	{
		...
		if (_resetOptions)
		{
			...
			var _cssText = (''
				...
				+	'#box { width: 800px; background-color: [=color_background]; } '
...
/* ======= */
	...
	$R.getContent = function (checkIfHomePage)
	{
		...
		//	======================================
			...
			if ($R.debug) { $R.debugTimerStart('get/processTargetHTML'); }
				...
				//$R.debugOutputsFixed['TitleSource'] = 'Body';//这是一个bug
	...
	$R.getContent__manualSelection = function ()
	{
		...
		if ($R.getContent__nrWords(_text) > 15); else { return false; }
		
		//	got some selected text; show it
		$R.doc_content=$R.processHTML(_html);
		$R.displayHTML($R.doc_content);
...
/* ========= */
	...
	$R.getContent__findHere__findTitleInHTML = function (_html, _rule_set)
	{
		...
		for (var i=0, _i=_possibles.length; i<_i; i++)
		{
			...
			{
				//	make it into a H1
				_html = _html.replace((new RegExp('<'+_possibles[i]+'[^>]*>([\\s\\S]+?)</'+_possibles[i]+'>', 'i')), '<h2 class="found_title">$1</h2>');
	...
	$R.getContent__nrWords = function (_text)
	{
		...
		return (1 + (_words_match != null ? (_text.length) : 0));
...
/* ============== */
	...
	{
		$('#__art_title').val(window.parent.document.title);
		
		content_r = $R.doc_content;
		if(window.location.host=='wenku.baidu.com')//暂只支持word
		{
			c=$R.win.document.body.innerHTML;
			i=c.indexOf('reader-container-inner');
			b=c.indexOf('sampling-area');
			content_r=c.substring(i,b);
			
			content_r = content_r.replace(/<\/?div.*?>|<\/?b.*?>|<\/?span.*?>|<\/?style.*?>|<\/?script.*?>/ig, "");			
			content_r = content_r.replace(/(<p).*?>/g, "$1>");
			content_r = content_r.replace(/<\/p><p>([^&|^ ])/ig, "$1");//去掉多余p 1
			content_r = content_r.replace(/(\r|\n)/ig, "");
			content_r = content_r.replace(/((<\/?p>)(?!<\/?p>)*<\/?p>( |&nbsp;){1,3})+/ig, "<p>");//去掉多余p 2
			content_r = content_r.substring(content_r.indexOf('>')+1,content_r.lastIndexOf('<p>'));//删除首尾无用字符
			content_r = content_r.replace(/<h2 class="banner-title">.+?<p>/ig, "");//去广告			
		}
		rep_str='http://'+window.location.host+'/';
		content_r = content_r.replace(/(<img.+?src ?= ?['"])((?!http:\/\/)[^"]*)/mg, "$1"+rep_str+"$2");
		rep_str='?url='+rep_str;
		content_r = content_r.replace(/(<a.+?href ?= ?['"])((?!http:\/\/)[^"]*)/mg, "$1"+rep_str+"$2");
		content_r = content_r.replace(/(<a.+?)_blank/mg, "$1_self");
		$R.article_content=$('#__art_content');
		$R.editor=$('#__art_content').xheditor({skin:'nostyle'});
		$R.editor.setSource(content_r);
	}
	$R.show = function ()
	{
		//	debug
